...
## What is `2>&1` or How to redirect stderr to stdout
...
## How to kill for loop in shell script

While looping in a shell script if the user presses `Ctrl-C` it will skip to
the next iteration instead of terminating as expected. To resolve this issue
add the following command at the start of the shell script.

    trap "exit" SIGINT

This command traps the `SIGIN` singal and excutes the `exit` command as
specified.
